...
- [ ] **v2 architecture, training in progress**: since *train* drift plateaued too (not just val, across 6 warm-restart cycles on v1), that's underfitting — capacity/receptive-field limited, not overfitting — so v1's 214K-param CNN+GRU (32/64/64 channels, 128-hidden 2-layer GRU) was replaced with a dilated residual CNN (TCN-style, 64/128/256 channels, dilation 1/2/4) + deeper unidirectional GRU (256-hidden, 3 layers) — 1.9M params (~7.6MB fp32). GRU stays unidirectional deliberately (not bidirectional) so it still runs causally on-device as samples stream in, rather than needing a whole window's future before correcting its start. Full rationale: [src/models/bias_correction_net.py](src/models/bias_correction_net.py)'s module docstring. Not yet trained/evaluated — v1's `checkpoints/best_6ch_baseline.pt` (62.76% test drift) stays the known-good fallback in the meantime.
