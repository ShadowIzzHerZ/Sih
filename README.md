...
- [ ] **Now retraining with 6 added engineered input channels** (accel/gyro magnitude, jerk, local smoothing+roughness — see `windowing.py`'s `engineer_features()`) instead of just 6 raw axes, on the theory that the plateau above was a signal-starvation problem, not an optimization one. `checkpoints/best.pt` / `results/eval_report.json` will reflect this 12-channel model once a run completes — old artifacts are the `*_6ch_baseline` files above.
- [ ] ONNX export — re-export once the 12-channel retrain produces a checkpoint (`checkpoints/dead_reckoning_model.onnx` was deleted along with the incompatible 6-channel checkpoint; export was previously verified to match PyTorch to 4.3e-6 and that mechanism hasn't changed, just the input shape)
